...
Rocket.LaunchAngle = launch_ang; % ANGLE IN DEGREES [0, 180]
...
StateVecInit = [
 [0 0 Rocket.LaunchAlt]' + Rocket.InitDistAlongRail*Rocket.LaunchVec;   % Position Vector
 cosd(Rocket.LaunchAngle/2); sind(Rocket.LaunchAngle/2)*[0; -1; 0];     % Quaternion Vector
 zeros(3,1);                                                            % Linear Velocity Vector
 zeros(3,1)];                                                           % Angular Velocity Vector
Func = @(Time,StateVec) RocketDynModel(StateVec,Time,Global,Rocket);
[Time,StateVecTraj] = ode23(...
